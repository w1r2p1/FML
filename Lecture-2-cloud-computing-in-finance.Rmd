...
---
class:middle
# Integrating `r fa('python')` and`r fa('r-project')`
...
```{r}
library(reticulate)
use_python("/Users/barry/opt/anaconda3/bin/python3") # If you are running this locally you need to point to the binary version of python on your machine.  One the server this has been done automatically for you so you do not need to run this line.
```
...
.panel[.panel-name[Python Code Run MC simulator]
```{python,time_it=TRUE}
I=100
M=100
...
```{r plot_python_object, echo=FALSE, width="100%",height="50%"}
py$S %>% 
  as_tibble() %>%
  mutate(t=row_number()) %>%
  gather(Path,Price,-t) %>%
