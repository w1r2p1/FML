...
- .salt[Economics of AI in Business]
---
class:middle
...
---
class:top
### Evolution of finance complexity and cloud computing: *Early years*

```{r cloud_timeline1, echo=FALSE,out.width="100%",out.height="50%"}

c("Black-Scholes-Merton<br>options pricing model", 
              "Stochastic modelling<br>of volatility" ,
              "Jump diffusion models")->financeMs

            c("Late Professor JohnMcCarthy (MIT) suggested<br>computing would be sold as a utility",
              ...
              "OpenNebula research project launched;<br> Big data starts to become popular",
              "Amazon launch Elastic Compute Cloud (EC2)",
              "Dropbox founded<br>; Cloud storage becomes a service")->cloudMs
content<-c(financeMs,cloudMs)            
start   = c("1973","1993","2000","1961","1967","1969","1988","1991","1996","2002","2005","2006","2007")
...
timevis(
  ...
  start=start,
  content =content ,
  group=c(rep(1,i),rep(2,j)),
  style=c(rep("font-size: 0.5em; color: blue;",i),rep("font-size: 0.5em; color: red;",j))),
  groups=data.frame(id = 1:2, content = c("Computational<br>Milestones<br>in<br>Finance","Cloud<br>Computing<br>Milestones"))) %>%
  fitWindow(options=list(animation=FALSE))
```
---
class:top
### Evolution of financial problem complexity and cloud computing *last decade*

```{r cloud_timeline2, echo=FALSE,out.width="100%",out.height="50%"}
c("Flash Crash",
              "Basel III",
              "Rebentrost et al.2018<br> propose a quantum computational<br> algo to price derivatives",
              "CME Smart Stream launched:<br> First derivatives exchange<br>to offer real-time cloud-based market data")->financeMs
c(
              "Microsoft launch Azure",
              "DigitalOcean launched; <br>third largest cloud computing company (2020)",
              ...
              "Google Tensor Processing Units <br>(TPUs) avaliable on the Cloud")->cloudMs 
content<-c(financeMs,cloudMs)
start   = c("2010","2013","2018","2019","2010","2012","2013","2015","2018","2019")
...
.pull-right[
![](img/workflow_company.png)
]


- Firstly, the workflow should be broken down into "jobs" or "tasks".  Each of these tasks usually has a human decision or action attached, which has the potential to be automated via an AI tool.  For instance, the CPW workflow has two critical decisions:
...
2.	How can the price be efficiently and dynamically retrieved?
 A successful corporate AI strategy requires clear and meaningful decomposition of the workflow.  Each task in the workflow needs to be decomposed in order to see where AI prediction can be inserted.  This, in turn, allows estimation of the benefit of enhanced prediction and the cost of generating that prediction.  Once reasonable estimates have been made, the potential AI projects can be rank-ordered from highest to lowest return on investment.  Management can then start at the top and implement AI tools as long as the expected ROI makes sense.
